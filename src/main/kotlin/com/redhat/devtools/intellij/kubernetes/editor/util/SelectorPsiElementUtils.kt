...
fun PsiElement.getLabels(): PsiElement? {
    return when(this) {
        is YAMLMapping -> getLabels()
        is JsonObject -> getLabels()
        else -> null
    }
}
...
private fun YAMLMapping.areMatchingMatchLabels(labels: YAMLMapping): Boolean {
    ...
    return matchLabels.keyValues.isNotEmpty()
            && matchLabels.keyValues.all { matchLabel ->
...
private fun YAMLMapping.areMatchingMatchExpressions(labels: YAMLMapping): Boolean {
    ...
    return expressions.items.isNotEmpty()
            && expressions.items.all { expression ->
