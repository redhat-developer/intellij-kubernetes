...
fun PsiElement.getKubernetesTypeInfo(): KubernetesTypeInfo? {
    return when {
        this is PsiFile || this is YAMLDocument -> null
...
fun PsiElement.getSelectorKeyElement(): PsiElement?{
    return when(this) {
        is YAMLMapping -> this.getSelectorKeyElement()
        is JsonObject -> this.getSelectorKeyElement()
        else -> null
    }
}

private fun YAMLMapping.getSelectorKeyElement(): PsiElement? {
    return (this.getSelector()?.parent as? YAMLKeyValue)?.key
}

private fun JsonObject.getSelectorKeyElement(): PsiElement? {
    return (this.getSelector()?.parent as? JsonProperty)?.nameElement
}

fun PsiElement.getTemplate(): PsiElement? {
    return when(this) {
        is YAMLMapping ->
            (this.getKeyValueByKey(KEY_SPEC)?.value as? YAMLMapping)
                ?.getKeyValueByKey(KEY_TEMPLATE)?.value as? YAMLMapping
        is JsonObject ->
            (this.findProperty(KEY_SPEC)?.value as? JsonObject?)
