...
fun createKeyValue(key: String, value: String? = null, parent: YAMLMapping? = null, project: Project = mock()): YAMLKeyValue {
	val valueElement = mock<YAMLValue> {
		on { text } doReturn value
	}
	return createKeyValue(key, valueElement, parent, project)
}

fun createKeyValue(key: String, value: YAMLValue, parent: YAMLMapping?, project: Project = mock()): YAMLKeyValue {
	val keyElement = mock<PsiNamedElement> {
		on { name } doReturn key
	}
	return createKeyValue(keyElement, value, parent, project)
}

fun createKeyValue(key: PsiNamedElement, value: YAMLValue, parent: YAMLMapping?, project: Project = mock()): YAMLKeyValue {
...
fun createYAMLMapping(key: String, value: String, parent: YAMLMapping? = null, project: Project = mock()): YAMLMapping {
	val keyValue = createKeyValue(key, value)
	return createYAMLMapping(listOf(keyValue))
}

fun createYAMLMapping(children: Collection<YAMLKeyValue>): YAMLMapping {
	return mock<YAMLMapping> {
		on { mock.keyValues } doReturn children
...
fun createProperty(name: String, value: String, parent: JsonObject? = null, project: Project = mock()): JsonProperty {
	val nameElement = mock<JsonValue> {
		on { mock.name } doReturn name
	}
	val valueElement = mock<JsonObject> {
		on { mock.name } doReturn name
		on { mock.text } doReturn value
	}
	return createProperty(nameElement, valueElement, parent, project)
}

fun createProperty(name: String, value: JsonObject, parent: JsonObject?, project: Project = mock()): JsonProperty {
	val nameElement = mock<JsonValue> {
		on { mock.name } doReturn name
	}
	return createProperty(nameElement, value, parent, project)
}

fun createProperty(nameElement: JsonValue, value: JsonObject, parent: JsonObject?, project: Project = mock()): JsonProperty {
