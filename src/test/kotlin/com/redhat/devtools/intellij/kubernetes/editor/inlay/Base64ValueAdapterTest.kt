...
import com.redhat.devtools.intellij.kubernetes.editor.mocks.createJsonObjectFor
...
class Base64ValueAdapterTest {
	...
	fun `#get should return value of YAMLKeyValue`() {
		// given
		val element = createKeyValueFor("jedi", value = "yoda", project = project)
	...
	fun `#get should return value with quotes`() {
		// given
		val element = createKeyValueFor(key = "jedi", value = "\"yoda\"", project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		val text = adapter.get()
		// then
	...
	fun `#get should return value of JsonProperty`() {
		// given
		val element = createPropertyFor("jedi", "yoda", project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		val text = adapter.get()
		// then
	...
	fun `#getDecoded should return value decoded value`() {
		// given
		val element = createKeyValueFor("jedi", value = toBase64("skywalker"), project = project)
		val adapter = Base64ValueAdapter(element)
		// when
	...
	fun `#getDecoded should return null if value isn't valid base64`() {
		// given
		val element = createKeyValueFor("anakin", toBase64("skywalker") + "bogus", project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		val text = adapter.getDecoded()
		// then
	...
	fun `#getDecoded should return null if value is null`() {
		// given
		val element = createKeyValueFor("c-3po", null, project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		val text = adapter.getDecoded()
		// then
	...
	fun `#getDecoded should return value without quotes`() {
		// given
		val element = createKeyValueFor("jedi", value = "\"" + toBase64("yoda") + "\"", project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		val text = adapter.getDecoded()
		// then
	...
	fun `#getDecodedBytes should return null for null value`() {
		// given
		val element = createKeyValueFor("jedi", value = null, project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		val bytes = adapter.getDecodedBytes()
		// then
	...
	fun `#set should add new YAMKeyValue to parent and delete current element`() {
		...
		val element = createKeyValueFor("jedi", "yoda", parent, project)
		val adapter = Base64ValueAdapter(element)
		// when
		adapter.set("luke")
		// then
	...
	fun `#set should create new YAMKeyValue with same key and given base64 encoded value`() {
		// given
		val parent = createYAMLMapping("group", "jedis")
		val element = createKeyValueFor("jedi", "yoda", parent, project = project)
		val adapter = Base64ValueAdapter(element)
		// when
		adapter.set("obiwan")
		// then
	...
	fun `#set should create new multiline value if existing value is multiline`() {
		// given
		val parent = createYAMLMapping("group", "jedis")
		val element = createKeyValueFor("jedi", "|\nyoda", parent, project)
		val adapter = Base64ValueAdapter(element)
		// when
		adapter.set("obiwan")
		// then
	...
	fun `#set should add new JsonProperty to parent and delete current element`() {
		...
		val parent = createJsonObjectFor("group", parentProperties, project = project)
		val property = createPropertyFor("jedi", "yoda", parent, project)
