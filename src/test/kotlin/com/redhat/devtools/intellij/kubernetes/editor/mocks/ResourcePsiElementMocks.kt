...
fun createLabels(labels: List<YAMLKeyValue>): YAMLMapping {
...
fun createLabels(labels: List<YAMLKeyValue>, parent: YAMLMapping): YAMLKeyValue {
    val labelMappings = createYAMLMapping(labels)
    return createLabels(labelMappings, parent)
}

fun createLabels(labelsChildren: YAMLMapping, parent: YAMLMapping): YAMLKeyValue {
    val metadataChildren = mock<YAMLMapping>()
    createKeyValue("metadata", metadataChildren, parent)
    return createKeyValue("labels", labelsChildren, metadataChildren)
}
...
fun createMatchLabelsFor(matchLabels: YAMLMapping, parent: YAMLMapping): YAMLKeyValue {
    ...
    return createKeyValue("matchLabels", matchLabels, selectorChildren)
}

fun createMatchExpressionsFor(matchExpressionsChildren: YAMLSequence, parent: YAMLMapping): YAMLKeyValue {
    val selectorChildren = mock<YAMLMapping>()
    createSelectorFor(selectorChildren, parent)
    return createKeyValue("matchExpressions", matchExpressionsChildren, selectorChildren)
}
...
fun createSelectorFor(selectorChildren: YAMLMapping = mock(), parent: YAMLMapping): YAMLKeyValue {
    ...
    createKeyValue("spec", specChildren, parent)
    return createKeyValue("selector", selectorChildren, specChildren)
}

fun createSelectorFor(selectorChildren: JsonObject = mock(), parent: JsonObject): JsonProperty {
    val specChildren = mock<JsonObject>()
    createProperty("spec", specChildren, parent)
    return createProperty("selector", selectorChildren, specChildren)
}
...
fun createMetadataFor(parent: JsonObject): JsonObject {
    ...
    createProperty("metadata", metadataChildren, parent)
    return metadataChildren
}
